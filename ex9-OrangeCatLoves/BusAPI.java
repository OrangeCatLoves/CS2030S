...
class BusAPI {
  ...
  private static CompletableFuture<String> httpGet(String url) {
    ...
    response = client.sendAsync(request, BodyHandlers.ofString())
      .handle((result, e) -> { // Exception handling
    ...
    // Second if condition statement, do thenApply.
    // Similar to mapping the value inside CompletableFuture to 
    // response.body()
    CompletableFuture<String> output = response.thenApply(x -> {
      if (x.statusCode() != 200) {
