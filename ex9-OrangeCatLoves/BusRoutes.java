...
class BusRoutes {
  ...
  public CompletableFuture<String> description() {
    ...
    // thenCombine same as combiner, maps each output to combined output inside CompletableFuture.
    for (BusService service : services.keySet()) {
      CompletableFuture<Set<BusStop>> stop = services.get(service);
      output = output.thenCombine(stop, (str, set) -> str + describeService(service, set));
    }
    return output;
  }
